...
const courseDetailsReducer = (state,action) => {
    ...
    if(action.type==='REMOVE_SUBJECT') {
        state.semesters.find((_,i) => i === action.payload.semIndex).subjects.splice(action.payload.subIndex,1);
        return {...state};
    }
    ...
    return(
       <div className="create-course-main">
        <div className="back-btn flex" onClick={() => navigate(-1)}>
            <FiArrowLeft color="var(--light-grey)" className="create-svg" size={25} /> 
            <span>Back</span>
        </div>
            ...
            <form onSubmit={newCourseSubmit}>
                ...
                <div className="semester-wrapper">
                    <div className="semester-header">
                        <h2>Semesters</h2>
                        <button className="btn-outlined new-sem-btn flex" onClick={addSem}>
